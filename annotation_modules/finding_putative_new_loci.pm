...
sub finding_putative_new_loci {
  ...
  while(my $org=<ORGANISMS>) {
    ...
    foreach my $unique_hit(@unique_hits){
        ...
        #split on colons
        my @hit_infor=split /:/, $unique_hit;

        #$hit=$hit_id."+".":".$hit_start.":".$hit_end;

        #Get hit id, start, end
        my $unique_hit_id=$hit_infor[0];
        my $unique_hit_start=$hit_infor[1];
        my $unique_hit_end=$hit_infor[2];

        my $strand='';

        #Set strand to positive or negative, same as extracted bac
        if($unique_hit_id=~/\+/){
            $strand="+";
        }
        else{
